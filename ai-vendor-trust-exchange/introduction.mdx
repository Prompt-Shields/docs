...
| The publish-time gate, the source registry, and the [field-level re-verification rules](/ai-vendor-trust-exchange/trust-profiles#what-may-advance-a-date) | **Give-to-get unlocking** — contributing does not yet unlock anything |
| Scheduled re-crawling, typed change events, the public change log and change feeds, and staleness surfacing | Trust ratings, crowd reviews, and the vendor portal |
| Member accounts, contribution intake, the shared normalization schema, model-assisted mapping, and the human review queue | — |
