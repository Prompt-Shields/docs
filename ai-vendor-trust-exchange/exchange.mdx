...
<Note>
**Mostly built — except the part that makes it an exchange.** Member accounts, contribution intake, the shared normalization schema, the model-assisted mapping and the human review queue all exist. What does not exist is the **unlocking**: entitlements still deny everyone, so contributing earns nothing back yet, and every reader sees the free summary. Watchlists and alerts are also outstanding. Until the gate opens this is a contribution pipeline rather than a give-to-get exchange, and the sections below describe the design it is being built toward.
</Note>
