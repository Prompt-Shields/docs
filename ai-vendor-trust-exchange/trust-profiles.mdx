...
<Warning>
**Partly built.** Every published field already carries a `last_verified_at` date, and the publish gate refuses a field without one. The machinery that *advances* those dates — scheduled re-crawling, change detection, the public change log and watchlist alerts — is designed and not yet built. Today a date tells you when an analyst confirmed the fact, and nothing re-confirms it automatically.
</Warning>
